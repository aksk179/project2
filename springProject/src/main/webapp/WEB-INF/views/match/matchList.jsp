...
<div id="game">
  ...
  <c:when test="${empty loginMember.userId}">
    <button id="challengeButton" onclick="challenge('${matchList.userId}', '${loginMember.userId}', '${loginMember.userStatus}', '${matchList.no}', '${matchList.matchdatestring}', '${matchList.matchtime}', '${matchList.gymName}', '${matchList.proNick}');">매칭도전신청</button>
  </c:when>
  <c:otherwise>
    <c:choose>
...
<script>
	...
	function challenge(userId1, userId2, userStatus, no, matchdateString, matchTime, gymName, proNick1) {
	    if(userId2 === "") {
	    	...
	    	if (!confirm("신청하시겠습니까?")) {
	    		...
	    		const url = "${pageContext.request.contextPath}/match/challengerAdd.ma?no=" + no + "&userId1=" + userId1 + "&userId2=" + userId2 
	    					+ "&matchdateString=" +  matchdateString + "&matchTime=" + matchTime + "&gymName=" + gymName + "&proNick1=" + proNick1;
		    	location.href = url;
	    	}
	    }	    
	}
